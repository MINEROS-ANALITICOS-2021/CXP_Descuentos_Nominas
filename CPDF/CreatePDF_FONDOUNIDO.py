...
class CreatePDFFondounido(FPDF):
    ...
    def clean_csvFU(self) -> str:
        try:
            ...
            df['Acount_Deudor2'] = df['Acount_Deudor2'].astype(str).str.replace('.0', '', regex=False)

            if 'Nombre' in df.columns and 'Status' in df.columns:
                df_filter = df[(df['Nombre'] == 'FONDO UNIDO DE GUATEMALA') & (df['Status'] == 'ok')].copy()
            ...
            for _, fila in df_filter.iterrows():
                generar_comprobanteFU(fila, carpeta)

        except Exception as e:
            print(f"Error en el proceso: {e}")


    def convertir_numero_a_letrasFU(self, numero):
        entero = int(numero)
    ...
    def agregar_cuerpoFU(self, Soc, Proveedor, Nombre, N_doc, Usuario, Texto, Fe_contab, Importe, ML, Key, Nom_Soc, Acount_Deudor1, Acount_Deudor2, Deudor_Inter1, nameCoor):
        ...
        self.set_xy(60, 80)
        self.set_font("Arial", "", 11)
        self.cell(0, 10, f"{self.convertir_numero_a_letrasFU(Importe)} /100", 0, 1)
        ...
        self.set_xy(60, 110)
        self.set_font("Arial", "", 11)
        self.multi_cell(0, 8, f"GUATEMALA >>>>  {fecha_formateada.upper()}", 0)
        ...
        self.set_xy(10, 185)
        self.set_font("Arial", "B", 6)
        self.cell(0, 10, f"40 {Acount_Deudor1}", 0, 1)

        # Text Value 2
        self.set_xy(10, 190)
        self.set_font("Arial", "B", 6)
        self.cell(0, 10, f"40 {Acount_Deudor2}", 0, 1)

        # Número aleatorio entre 000 y 999 con ceros a la izquierda
...
def generar_comprobanteFU(datos, carpeta):
    pdf = CreatePDFFondounido(path_Receipts=carpeta, file_ReportPDF="", name_coord="")
    pdf.add_page()
    pdf.agregar_cuerpoFU(
        datos["Soc"],
        datos["Proveedor"],
        datos["Nombre"],
        datos["N_doc"],
